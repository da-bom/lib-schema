...
> **작성자**: DABOM 팀
> **변경 이력**:
> - v24.0 - usage-events 처리 흐름 및 notification outbox 구조를 함께 재정의: `usage-persist`/`usage-realtime` 토픽 제거, processor-usage가 Redis/Lua 이후 `usage_record`·`customer_quota`·`family_quota`를 직접 DB 정산하는 구조로 동기화. `usage_event_outbox` 신규 테이블 추가, notification payload 평탄화, 배치 서버의 `notification-events` 후행 발행 흐름, FK(`family_id`, `customer_id`), UNIQUE(`event_id`), 재시도 인덱스(`status`, `next_retry_at`)를 반영하고 엔티티 총 23→24개로 확장
...
| `usage_event_outbox` | `customer_id` | `customer.id` | 기본 FK 정책 |
| `usage_event_outbox` | `family_id` | `family.id` | 기본 FK 정책 |
